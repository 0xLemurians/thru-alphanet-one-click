...
readonly NVM_INSTALLER_VERSION="0.40.3"
...
require_root() {
  [[ ${EUID} -eq 0 ]] ||
    die "Bu script root olarak çalıştırılmalı: sudo -H bash install.sh"

  [[ ${HOME} == "/root" ]] ||
    warn "HOME=${HOME}. Önerilen kullanım: sudo -H bash install.sh"
}
...
require_supported_host() {
  ...
  [[ ${ID:-} == "ubuntu" ]] ||
    die "Yalnızca Ubuntu destekleniyor. Algılanan: ${ID:-bilinmiyor}"

  [[ ${VERSION_ID:-} == "24.04" ]] ||
    die "Ubuntu 24.04 gerekli. Algılanan: ${VERSION_ID:-bilinmiyor}"

  [[ $(uname -m) == "x86_64" ]] ||
    die "x86_64 gerekli. Algılanan: $(uname -m)"

  local free_kb
  free_kb="$(df --output=avail / | tail -1 | tr -d ' ')"

  (( free_kb >= 8 * 1024 * 1024 )) ||
    die "En az 8 GB boş disk alanı gerekli."
}
